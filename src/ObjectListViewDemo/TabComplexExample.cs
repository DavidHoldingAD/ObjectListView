...
public partial class TabComplexExample : OlvDemoTab
{
	...
	protected override void InitializeTab()
	{
		...
		olvComplex.SetObjects(Coordinator.PersonList);
	}
	...
	private void SetupListView()
	{
		...
		olvComplex.TileSize = new Size(250, 120);

		// Do some fancy editing
		columnCookingSkill.CellEditUseWholeCell = true;
		olvComplex.CellEditStarting += listViewComplex_CellEditStarting;
		olvComplex.CellEditFinishing += listViewComplex_CellEditFinishing;
		olvComplex.CellEditValidating += listViewComplex_CellEditValidating;

		// Put love heart decorations on one of the items
		olvComplex.UseCellFormatEvents = true;
		olvComplex.FormatRow += olvComplex_FormatRow;
		olvComplex.FormatCell += olvComplex_FormatCell;
		...
		olvComplex.BeforeCreatingGroups += delegate (object sender, CreateGroupsEventArgs e)
		{
			if (e.Parameters.PrimarySort == olvMarriedColumn && Control.ModifierKeys == Keys.Control)
			{
				...
				e.Parameters.ItemComparer = new StrangeItemComparer(olvComplex.GetColumn(0), e.Parameters.PrimarySortOrder);
			}
		};
		...
		olvComplex.IsHyperlink += delegate (object sender, IsHyperlinkEventArgs e)
		{
		...
		olvComplex.CellRightClick += delegate (object sender, CellRightClickEventArgs cellRightClickEventArgs)
		{
			ContextMenuStrip ms = new();
			ObjectListView olv = cellRightClickEventArgs.ListView;
			if (olv.ShowGroups)
			{
				foreach (ListViewGroup lvg in olv.Groups)
				{
					ToolStripMenuItem mi = new(string.Format("Jump to group '{0}'", lvg.Header));
					mi.Tag = lvg;
					ms.Items.Add(mi);
				}
			}
			...
			{
				ToolStripMenuItem mi = new("Turn on 'Show Groups' to see this context menu in action");
				mi.Enabled = false;
				ms.Items.Add(mi);
			}
	...
	private void SetupControls()
	{
		...
		comboBoxHotItemStyle.SelectedIndex = 3; // Translucent
		comboBoxView.SelectedIndex = 4; // Details
		comboBoxEditable.SelectedIndex = 0; // Not editable
	}
	...
	private void SetupDecorations()
	{
		...
		olvComplex.AddDecoration(new EditingCellBorderDecoration(true));
		...
		olvComplex.ItemRenderer = new BusinessCardRenderer();
		olvComplex.TileSize = new Size(230, 120);
	}
	...
	private void SetupColumns()
	{
		...
		personColumn.AspectToStringConverter = delegate (object cellValue)
		...
		personColumn.ImageGetter = delegate (object row)
		{
		...
		columnCookingSkill.MakeGroupies(
		...
		hourlyRateColumn.MakeGroupies(
			new double[] { 100, 1000 },
			new string[] { "Less than $100", "$100-$1000", "Megabucks" });
		hourlyRateColumn.AspectPutter = delegate (object x, object newValue)
		...
		moneyImageColumn.AspectGetter = delegate (object row)
		{
		...
		moneyImageColumn.Renderer = new MappedImageRenderer(new object[] {
		...
		birthdayColumn.GroupKeyGetter = delegate (object row)
		{ return ((Person)row).BirthDate.Month; };
		birthdayColumn.GroupKeyToTitleConverter = delegate (object key)
		{ return (new DateTime(1, (int)key, 1)).ToString("MMMM"); };
		birthdayColumn.ImageGetter = delegate (object row)
		{
			Person p = (Person)row;
		...
		birthdayColumn.ClusteringStrategy = new DateTimeClusteringStrategy(DateTimePortion.Month, "MMMM");

		// Use this column to test sorting and group on TimeSpan objects
		daysSinceBirthColumn.AspectGetter = delegate (object row)
		{ return DateTime.Now - ((Person)row).BirthDate; };
		daysSinceBirthColumn.AspectToStringConverter = delegate (object aspect)
		{ return ((TimeSpan)aspect).Days.ToString(); };

		// Give the Tells Jokes? column better grouping labels
		olvJokeColumn.GroupKeyToTitleConverter = delegate (object groupKey)
		{
	...
	private void SetupDragAndDrop()
	{
		...
		olvComplex.DragSource = new SimpleDragSource();
		...
		{
			if (e.TargetModel is not Person person)
			{
	...
	private void SetupFasterAspectGetters()
	{
		...
		TypedObjectListView<Person> tlist = new(olvComplex);
	...
	private void SetupDarkerStyle()
	{
		...
		olvComplex.UseAlternatingBackColors = true;
		olvComplex.BackColor = Color.FromArgb(30, 30, 40);
		olvComplex.AlternateRowBackColor = Color.FromArgb(35, 35, 42);
		olvComplex.ForeColor = Color.WhiteSmoke;

		HyperlinkStyle hls = new();
		hls.Normal = new CellStyle();
		hls.Normal.FontStyle = FontStyle.Underline;
		olvComplex.HyperlinkStyle = hls;

		SimpleItemStyle dis = new();
		dis.ForeColor = Color.Gray;
		dis.BackColor = Color.FromArgb(30, 30, 35);
		dis.Font = new Font("Stencil", 10);
		olvComplex.DisabledItemStyle = dis;
	}
	...
	private void SetupTooltips()
	{
		...
		olvComplex.HeaderToolTipShowing += delegate (object sender, ToolTipShowingEventArgs e)
		{
		...
		olvComplex.CellToolTipShowing += delegate (object sender, ToolTipShowingEventArgs e)
		{
	...
	private void olvComplex_FormatCell(object sender, FormatCellEventArgs e)
	{
		...
		{
			TextDecoration decoration = new("Missing!", 255);
	...
	private void textBoxFilterComplex_TextChanged(object sender, EventArgs e) => Coordinator.TimedFilter(ListView, ((TextBox)sender).Text);
	...
	{
		Coordinator.ChangeView(ListView, (ComboBox)sender);

		// Make the hot item show an overlay when it changes
		if (olvComplex.UseTranslucentHotItem)
		{
			olvComplex.HotItemStyle.Overlay = new BusinessCardOverlay();
...
public class StrangeItemComparer : IComparer<OLVListItem>
{
	...
	public int Compare(OLVListItem x, OLVListItem y)
	{
		...
		string xValue = column.GetStringValue(x.RowObject);
		string yValue = column.GetStringValue(y.RowObject);
