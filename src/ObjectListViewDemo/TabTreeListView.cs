...
public partial class TabTreeListView : OlvDemoTab
{
	...
	public TabTreeListView()
	{
		...
		ListView.CellClick += (sender, args) => Debug.WriteLine("CellClicked: {0}", args);
	}
	...
	protected override void InitializeTab()
	{
		...
		treeListView.HierarchicalCheckboxes = checkBoxHierarchicalCheckboxes.Checked;
		comboBoxExpanders.SelectedIndex = 2; // triangles
		...
		TreeListView.TreeRenderer renderer = treeListView.TreeColumnRenderer;
	...
	private void SetupColumns()
	{
		...
		SysImageListHelper helper = new(treeListView);
	...
	{
		object model = treeListView.SelectedObject;
		if (model != null)
		{
			treeListView.ToggleExpansion(model);
		}
	}

	private void checkBoxHierarchicalCheckboxes_CheckedChanged(object sender, EventArgs e) => treeListView.HierarchicalCheckboxes = ((CheckBox)sender).Checked;

	private void textBoxFilter_TextChanged(object sender, EventArgs e) => Coordinator.TimedFilter(ListView, ((TextBox)sender).Text);
	...
	private void buttonRefresh_Click(object sender, EventArgs e) => treeListView.RefreshObjects(treeListView.SelectedObjects);

	private void buttonCheck_Click(object sender, EventArgs e) => treeListView.ToggleSelectedRowCheckBoxes();
	...
	private void buttonRestoreState_Click(object sender, EventArgs e) => treeListView.RestoreState(treeListViewViewState);

	private void buttonColumns_Click(object sender, EventArgs e)
	{
		ColumnSelectionForm form = new();
		form.OpenOn(treeListView);
	}
	...
	{
		TreeListView.TreeRenderer treeColumnRenderer = treeListView.TreeColumnRenderer;
