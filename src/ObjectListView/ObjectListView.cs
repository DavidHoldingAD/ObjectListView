...
public partial class ObjectListView : ListView, ISupportInitialize
{
	...
	public virtual IList CheckedObjects
	{
		...
		set
		{
			...
			Hashtable table = new(GetItemCount());
			if (value != null)
			{
	...
	new public ListView.ColumnHeaderCollection Columns => base.Columns;
	...
	static public EditorRegistry EditorRegistry = new();
	...
	[Browsable(false),
	DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
	public FilterMenuBuilder FilterMenuBuildStrategy { get; set; } = new();
	...
	[Browsable(false),
	 DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
	new public ListViewGroupCollection Groups => base.Groups;
	...
	[Browsable(false),
	DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
	virtual public bool IsFiltering => UseFiltering && (ModelFilter != null || ListFilter != null);
	...
	[Browsable(false),
	 DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
	new public ListViewItemCollection Items => base.Items;
	...
	private void SetGroupSpacing()
	{
		...
		NativeMethods.LVGROUPMETRICS metrics = new();
	...
	private BeforeSortingEventArgs BuildBeforeSortingEventArgs(OLVColumn column, SortOrder order)
	{
		...
		BeforeSortingEventArgs args = new(
...
#pragma warning restore 168
		...
		CreateGroupsEventArgs args = new(parms);
		...
		OLVColumn sortByColumn, SortOrder sortByOrder, OLVColumn secondaryColumn, SortOrder secondaryOrder)
	{
		...
		GroupingParameters parms = new(this, groupByColumn, groupByOrder,
	...
	protected virtual IList<OLVGroup> MakeGroups(GroupingParameters parms)
	{
		...
		NullableDictionary<object, List<OLVListItem>> map = new();
	...
	public virtual void BuildList(bool shouldPreserveState)
	{
		...
		try
		{
			...
			if (objectsToDisplay != null)
			{
				...
				List<ListViewItem> itemList = new(); // use ListViewItem to avoid co-variant conversion
				foreach (object rowObject in objectsToDisplay)
				{
					OLVListItem lvi = new(rowObject);
	...
	public virtual void ChangeToFilteredColumns(View view)
	{
		...
		if (view == View.Details || ShowHeaderInAllViews)
		{
			...
			List<OLVColumn> columnsInDisplayOrder = new(columns);
			columnsInDisplayOrder.Sort(delegate (OLVColumn x, OLVColumn y)
			{ return (x.LastDisplayIndex - y.LastDisplayIndex); });
			int i = 0;
	...
	public virtual void CopyObjectsToClipboard(IList objectsToCopy)
	{
		...
		OLVDataObject dataObject = new(this, objectsToCopy);
	...
	public virtual string ObjectsToHtml(IList objectsToConvert)
	{
		...
		OLVExporter exporter = new(this, objectsToConvert);
	...
	public virtual void InsertObjects(int index, ICollection modelObjects)
	{
		...
		try
		{
			...
			ItemsAddingEventArgs args = new(modelObjects);
			OnItemsAdding(args);
			if (args.Canceled)
			{
				return;
			...
			else
			{
				...
				foreach (object modelObject in modelObjects)
				{
					if (modelObject != null)
					{
						...
						OLVListItem lvi = new(modelObject);
	...
	protected OlvListViewHitTestInfo LowLevelHitTest(int x, int y)
	{
		...
		NativeMethods.LVHITTESTINFO lParam = new();
		lParam.pt_x = x;
		lParam.pt_y = y;
		...
		OlvListViewHitTestInfo olvListViewHitTest = new(hitItem, subItem, lParam.flags, group, lParam.iSubItem);
	...
	protected virtual void CalculateStandardHitTest(OlvListViewHitTestInfo hti, int x, int y)
	{
		...
		Size proposedSize = new(textBounds.Width, textBounds.Height);
	...
	public virtual void RemoveObjects(ICollection modelObjects)
	{
		...
		{
			// Give the world a chance to cancel or change the added objects
			ItemsRemovingEventArgs args = new(modelObjects);
	...
	public virtual void SetObjects(IEnumerable collection, bool preserveState)
	{
		...
		{
			Invoke((MethodInvoker)delegate
			{ SetObjects(collection, preserveState); });
			return;
		}

		// Give the world a chance to cancel or change the assigned collection
		ItemsChangingEventArgs args = new(objects, collection);
	...
	{
		ObjectListViewState olvState = new();
		...
		return converter.Serialize(olvState);
#else
	...
	public virtual bool RestoreState(byte[] state)
	{
		...
		try
		{
			...
			olvState = converter.Deserialize<ObjectListViewState>(state);
		}
		catch
		{
			return false;
	...
	protected virtual bool HandleBeginScroll(ref Message m)
	{
		...
		if (nmlvscroll.dx != 0)
		{
			...
			ScrollEventArgs args = new(ScrollEventType.EndScroll, scrollPositionH - nmlvscroll.dx, scrollPositionH, ScrollOrientation.HorizontalScroll);
			OnScroll(args);
		...
		if (nmlvscroll.dy != 0)
		{
			...
			ScrollEventArgs args = new(ScrollEventType.EndScroll, scrollPositionV - nmlvscroll.dy, scrollPositionV, ScrollOrientation.VerticalScroll);
			OnScroll(args);
		}
	...
	protected virtual bool HandleChar(ref Message m)
	{
		...
		BeforeSearchingEventArgs args = new(lastSearchString, start);
		...
		AfterSearchingEventArgs args2 = new(searchString, found);
		...
		switch (nmcustomdraw.nmcd.dwDrawStage)
		{
			...
			case CDDS_SUBITEMPREPAINT:
				...
				using (Graphics g = Graphics.FromHdc(nmcustomdraw.nmcd.hdc))
				{
					...
					DrawListViewSubItemEventArgs args = new(g, r, item, item.SubItems[0], rowIndex, 0,
	...
	protected virtual bool HandleGroupInfo(ref Message m)
	{
		...
		foreach (OLVGroup group in OLVGroups)
		{
			...
			{
				GroupStateChangedEventArgs args = new(group, (GroupState)nmlvgroup.uOldState, (GroupState)nmlvgroup.uNewState);
	...
	protected virtual bool HandleKeyDown(ref Message m)
	{
		...
		{
			ScrollEventArgs args = new(ScrollEventType.EndScroll,
	...
	{
		GroupExpandingCollapsingEventArgs args = new(group);
	...
	{
		ColumnClickEventArgs eventArgs = new(columnIndex);
	...
	public virtual ToolStripDropDown MakeColumnSelectMenu(ToolStripDropDown strip)
	{
		...
		{
			ToolStripMenuItem menu = new(MenuLabelColumns);
	...
	protected void AddItemsToColumnSelectMenu(ToolStripItemCollection items)
	{
		...
		List<OLVColumn> columns = new(AllColumns);
		...
		foreach (OLVColumn col in columns)
		{
			ToolStripMenuItem mi = new(col.Text);
	...
	private void RunWhenIdle(EventHandler eventHandler)
	{
		...
		if (!CanUseApplicationIdle)
		{
			...
			{ Application.RaiseIdle(EventArgs.Empty); }, null);
		}
	}
	...
	protected virtual void ResizeFreeSpaceFillingColumns(int freeSpace)
	{
		...
		List<OLVColumn> spaceFillingColumns = new();
		for (int i = 0; i < Columns.Count; i++)
		{
			OLVColumn col = GetColumn(i);
	...
	protected virtual bool SetObjectCheckedness(object modelObject, CheckState state)
	{
		...
		ItemCheckEventArgs ice = new(olvi.Index, state, olvi.CheckState);
		OnItemCheck(ice);
	...
	private void ChangeHeaderCheckBoxState(OLVColumn column, CheckState newState)
	{
		...
		HeaderCheckBoxChangingEventArgs args = new();
		args.Column = column;
	...
	public virtual void ToggleSubItemCheckBox(object rowObject, OLVColumn column)
	{
		...
		SubItemCheckingEventArgs args = new(column, ModelToItem(rowObject), column.Index, currentState, newState);
	...
	/// </summary>
	new public void Sort() => Sort(PrimarySortColumn, PrimarySortOrder);
	...
	protected virtual void MakeSortIndicatorImages()
	{
		...
		{
			Point pt1 = new(midX - deltaX, midY + deltaY);
			Point pt2 = new(midX, midY - deltaY - 1);
			Point pt3 = new(midX + deltaX, midY + deltaY);
		...
		{
			Point pt1 = new(midX - deltaX, midY - deltaY);
			Point pt2 = new(midX, midY + deltaY);
			Point pt3 = new(midX + deltaX, midY - deltaY);
	...
	{
		Bitmap bm = new(sz.Width, sz.Height);
...
#pragma warning restore 168
	...
	{
		Bitmap b = new(il.ImageSize.Width, il.ImageSize.Height);
		Graphics g = Graphics.FromImage(b);
		g.Clear(il.TransparentColor);
		Point location = new(b.Width / 2 - 5, b.Height / 2 - 6);
	...
	{
		HyperlinkClickedEventArgs args = new();
	...
	{
		CellRightClickEventArgs args = new();
		BuildCellEvent(args, e.Location);
	...
	public virtual Rectangle CalculateColumnVisibleBounds(Rectangle bounds, OLVColumn column)
	{
		...
		Rectangle columnBounds = new(sides.X, bounds.Top, sides.Y - sides.X, bounds.Bottom);
	...
	public void ConfigureAutoComplete(TextBox tb, OLVColumn column, int maxRows)
	{
		...
		Dictionary<string, bool> alreadySeen = new();
	...
	protected virtual void UpdateHotItem(OlvListViewHitTestInfo hti)
	{
		...
		HotItemChangedEventArgs args = new();
	...
	private GlassPanelForm FindGlassPanelForOverlay(IOverlay overlay) => glassPanels.Find(delegate (GlassPanelForm x)
																			  { return x.Overlay == overlay; });
	...
	virtual protected IEnumerable FilterObjects(IEnumerable originalObjects, IModelFilter aModelFilter, IListFilter aListFilter)
	{
		...
		// ReSharper disable PossibleMultipleEnumeration
		FilterEventArgs args = new(originalObjects);
		OnFilter(args);
