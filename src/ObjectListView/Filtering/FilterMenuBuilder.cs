...
public class FilterMenuBuilder
{
	...
	virtual protected List<ICluster> Cluster(IClusteringStrategy strategy, ObjectListView listView, OLVColumn column)
	{
		...
		NullableDictionary<object, ICluster> map = new();
		int count = 0;
	...
	virtual protected void SortClusters(IClusteringStrategy strategy, List<ICluster> clusters) => clusters.Sort();
	...
	{
		ToolStripCheckedListBox checkedList = new();
		...
		ToolStripMenuItem clearAll = new(CLEAR_ALL_FILTERS_LABEL, ClearFilteringImage, delegate (object sender, EventArgs args)
		{
			ClearAllFilters(column);
		});
		ToolStripMenuItem apply = new(APPLY_LABEL, FilteringImage, delegate (object sender, EventArgs args)
		{
			EnactFilter(checkedList, column);
		});
		ToolStripMenuItem subMenu = new(FILTERING_LABEL, null, new ToolStripItem[] {
