...
public class FastObjectListView : VirtualObjectListView
{
	...
	public override void MoveObjects(int index, ICollection modelObjects)
	{
		if (InvokeRequired)
		{
			...
			{ MoveObjects(index, modelObjects); });
			return;
		}
...
public class FastObjectListDataSource : AbstractVirtualListDataSource
{
	...
	public override int GetObjectIndex(object model)
	{
		...
		if (model != null && objectsToIndexMap.TryGetValue(model, out int index))
		{
			return index;
		}
	...
	public override void Sort(OLVColumn column, SortOrder sortOrder)
	{
		...
		{
			ModelObjectComparer comparer = new(column, sortOrder, listView.SecondarySortColumn, listView.SecondarySortOrder);
	...
	public override void UpdateObject(int index, object modelObject)
	{
		...
		int i = ObjectList.IndexOf(filteredObjectList[index]);
		if (i < 0)
		{
			return;
		}
	...
	/// </summary>
	public ArrayList ObjectList { get; private set; } = new();
	...
	private readonly Dictionary<object, int> objectsToIndexMap = new();
	...
	protected void FilterObjects()
	{
		...
		FilterEventArgs args = new(ObjectList);
