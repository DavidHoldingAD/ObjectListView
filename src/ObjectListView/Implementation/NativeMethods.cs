...
internal static class NativeMethods
{
	...
	public static void SelectAllItems(ListView list) => NativeMethods.SetItemState(list, -1, LVIS_SELECTED, LVIS_SELECTED);

	/// <summary>
	/// Deselect all rows on the given listview
	/// </summary>
	/// <param name="list">The listview whose items are to be deselected</param>
	public static void DeselectAllItems(ListView list) => NativeMethods.SetItemState(list, -1, LVIS_SELECTED, 0);
	...
	public static void DeselectOneItem(ListView list, int index) => NativeMethods.SetItemState(list, index, LVIS_SELECTED, 0);
	...
	/// <returns></returns>
	public static bool ChangeZOrder(IWin32Window toBeMoved, IWin32Window reference) => NativeMethods.SetWindowPos(toBeMoved.Handle, reference.Handle, 0, 0, 0, 0, SWP_ZORDERONLY);
	...
	/// <returns></returns>
	public static bool ChangeSize(IWin32Window toBeMoved, int width, int height) => NativeMethods.SetWindowPos(toBeMoved.Handle, IntPtr.Zero, 0, 0, width, height, SWP_SIZEONLY);
	...
	/// back colors. 
	/// </remarks>
	static public void SetSelectedColumn(ListView objectListView, ColumnHeader value) => NativeMethods.SendMessage(objectListView.Handle,
			LVM_SETSELECTEDCOLUMN, (value == null) ? -1 : value.Index, 0);

	static public int GetTopIndex(ListView lv) => (int)SendMessage(lv.Handle, LVM_GETTOPINDEX, 0, 0);

	static public IntPtr GetTooltipControl(ListView lv) => SendMessage(lv.Handle, LVM_GETTOOLTIPS, 0, 0);

	static public IntPtr SetTooltipControl(ListView lv, ToolTipControl tooltip) => SendMessage(lv.Handle, LVM_SETTOOLTIPS, 0, tooltip.Handle);
	...
	public static int GetGroupInfo(ObjectListView olv, int groupId, ref LVGROUP2 group) => (int)NativeMethods.SendMessage(olv.Handle, LVM_GETGROUPINFO, groupId, ref group);

	public static GroupState GetGroupState(ObjectListView olv, int groupId, GroupState mask) => (GroupState)NativeMethods.SendMessage(olv.Handle, LVM_GETGROUPSTATE, groupId, (int)mask);

	public static int InsertGroup(ObjectListView olv, LVGROUP2 group) => (int)NativeMethods.SendMessage(olv.Handle, LVM_INSERTGROUP, -1, ref group);

	public static int SetGroupInfo(ObjectListView olv, int groupId, LVGROUP2 group) => (int)NativeMethods.SendMessage(olv.Handle, LVM_SETGROUPINFO, groupId, ref group);

	public static int SetGroupMetrics(ObjectListView olv, LVGROUPMETRICS metrics) => (int)NativeMethods.SendMessage(olv.Handle, LVM_SETGROUPMETRICS, 0, ref metrics);

	public static void ClearGroups(VirtualObjectListView virtualObjectListView) => NativeMethods.SendMessage(virtualObjectListView.Handle, LVM_REMOVEALLGROUPS, 0, 0);
	...
	public static int HitTest(ObjectListView olv, ref LVHITTESTINFO hittest) => (int)NativeMethods.SendMessage(olv.Handle, olv.View == View.Details ? LVM_SUBITEMHITTEST : LVM_HITTEST, -1, ref hittest);
}
