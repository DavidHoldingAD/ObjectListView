...
public class FastListGroupingStrategy : AbstractVirtualGroups
{
	...
	public override IList<OLVGroup> GetGroups(GroupingParameters parmameters)
	{
		...
		int objectCount = 0;
		NullableDictionary<object, List<object>> map = new();
		...
		ModelObjectComparer sorter = new(primarySortColumn, parmameters.PrimarySortOrder,
		...
		foreach (object key in map.Keys)
		{
			...
			lvg.Contents = map[key].ConvertAll<int>(delegate (object x)
			{ return folv.IndexOf(x); });
