...
public class BaseRenderer : AbstractRenderer
{
	...
	{
		Rectangle r = new(outer.Location, inner.Size);
	...
	protected virtual Rectangle CalculateAlignedRectangle(Graphics g, Rectangle r)
	{
		...
		Rectangle contentRectangle = new(Point.Empty, CalculateContentSize(g, r));
	...
	protected virtual Size CalculateTextSize(Graphics g, string txt, int width)
	{
		...
		{
			Size proposedSize = new(width, int.MaxValue);
	...
	protected virtual void StandardHitTest(Graphics g, OlvListViewHitTestInfo hti, Rectangle alignedContentRectangle, int x, int y)
	{
		...
		if (ColumnIsPrimary && ListView.CheckBoxes)
		{
			...
			Rectangle r3 = new(r.X, checkBoxTop, checkBoxSize.Width, checkBoxSize.Height);
			width = r3.Width + 6;
	...
	protected virtual void DrawAlignedImage(Graphics g, Rectangle r, Image image)
	{
		...
		Rectangle imageBounds = new(r.Location, image.Size);
	...
	protected virtual int DrawCheckBox(Graphics g, Rectangle r)
	{
		...
		Size checkBoxSize = CalculateCheckBoxSize(g);
		Point checkBoxLocation = new(r.X, AlignVertically(r, checkBoxSize.Height));
		...
		if (il != null)
		{
			...
			if (selectorAsInt >= 0)
			{
				if (!IsPrinting)
				{
					...
					Rectangle r2 = new(r.X - Bounds.X, r.Y - Bounds.Y, r.Width, r.Height);
...
public class HighlightTextRenderer : BaseRenderer, IFilterAwareRenderer
{
	...
	protected virtual void DrawGdiPlusTextHighlighting(Graphics g, Rectangle r, string txt)
	{
		...
		List<CharacterRange> ranges = new(Filter.FindAllMatchedRanges(txt));
...
public class ImageRenderer : BaseRenderer
{
	...
	protected Timer Tickler
	{
		get
		{
			if (tickler == null)
			{
				tickler = new System.Threading.Timer(new TimerCallback(OnTimer), null, Timeout.Infinite, Timeout.Infinite);
			}
	...
	protected Image GetImageFromAspect()
	{
		...
		{
			using (MemoryStream stream = new((byte[])Aspect))
			{
				try
				{
	...
	protected void OnTimerInThread()
	{
		...
		if (ListView.View != System.Windows.Forms.View.Details || Column == null || Column.Index < 0)
		{
		...
		Rectangle updateRect = new(); // what part of the view must be updated to draw the changed gifs?
	...
	public override void Render(Graphics g, Rectangle r)
	{
		...
		else
		{
			...
			if (fillRect.Width > 0)
			{
				...
				{
					using (LinearGradientBrush gradient = new(frameRect, GradientStartColor, GradientEndColor, LinearGradientMode.Horizontal))
					{
...
public class MultiImageRenderer : BaseRenderer
{
	...
	public override void Render(Graphics g, Rectangle r)
	{
		...
		Rectangle singleImageRect = new(imageBounds.X, imageBounds.Y, imageScaledWidth, imageScaledHeight);
		Color backgroundColor = GetBackgroundColor();
...
public class FlagRenderer : BaseRenderer
{
	...
	protected override void HandleHitTest(Graphics g, OlvListViewHitTestInfo hti, int x, int y)
	{
		...
		foreach (int key in keysInOrder)
		{
			if ((v2 & key) == key)
			{
				...
				if (image != null)
				{
					Rectangle imageRect = new(pt, image.Size);
	...
	private List<int> keysInOrder = new();
	private Dictionary<int, object> imageMap = new();
}
...
public class DescribedTaskRenderer : BaseRenderer, IFilterAwareRenderer
{
	...
	protected virtual void DrawDescribedTask(Graphics g, Rectangle r, string title, string description, object imageSelector)
	{
		...
		{
			using (SolidBrush b = new(TitleColorOrDefault))
			{
		...
		{
			using (SolidBrush b = new(DescriptionColorOrDefault))
			{
